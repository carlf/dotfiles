...
#+BEGIN_SRC emacs-lisp
  (use-package gruvbox-theme
    :ensure t
    :config
    (set-frame-font "FantasqueSansMono-12")
    (menu-bar-mode -1)
    (tool-bar-mode -1)
    (scroll-bar-mode -1)
...
#+BEGIN_SRC emacs-lisp
  ...
  (setq mu4e-maildir (expand-file-name "~/Mail")
        mu4e-sent-folder "/photocarl/Sent"
        mu4e-drafts-folder "/photocarl/Drafts"
        mu4e-trash-folder "/photocarl/Trash"
        mu4e-get-mail-command "mbsync -a"
        mu4e-use-fancy-chars nil
        mu4e-change-filenames-when-moving t
        mu4e-maildir-shortcuts '(("/photocarl/INBOX" . ?i)
                                 ("/photocarl/Archive" . ?a)
                                 ("/photocarl/Sent" . ?s)
                                 ("/photocarl/Spam" . ?j)))
#+END_SRC
