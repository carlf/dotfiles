...
* GNUS
  This is handy for reading newsgroups. Maybe someday I'll be free of
  Exchange and can use it for my email as well.
#+BEGIN_SRC emacs-lisp
  (use-package gnus
    :ensure t
    :config
    (setq user-mail-address "carlf@photocarl.org"
          user-full-name "Carl Flippin"
          gnus-select-method '(nntp "news.giganews.com"))
